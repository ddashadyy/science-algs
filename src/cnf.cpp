...
CNF::CNF(std::size_t amount_variables)
{
    ...
    if (cnf_out.is_open())
    {
        auto cnf = generate_random_cnf(amount_variables, amount_variables + 20, amount_variables + 10);
        cnf_out << cnf;
...
std::vector<std::string> CNF::split_cnf() const
{
    ...
    if (this->str.empty()) return result;
...
void Candidate::evaluate_quality_function(const CNF& cnf)
{
    ...
    for (const auto& disjunct : disjuncts)
        if (eval_disjunct(disjunct)) ++quality_counter;
...
bool Candidate::eval_disjunct(const std::string& disjunct) const
{
    ...
    {
        while (pos < disjunct.length() && disjunct[pos] == ' ') pos++;
        if (pos >= disjunct.length()) break;
